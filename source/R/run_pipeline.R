...
source(file.path(r_dir, "18_view_outcome.R"))
source(file.path(r_dir, "19_view_mortality.R"))
...
step("3/5 views",    {
  ...
  view_2024(panel)
  view_access(panel)
  ## ⑦ 결과 · ② 소유 형태 (2026-09-03 신설)
  view_outcome(panel)
  ## ⑦ 결과의 두 번째 축 - 시군구 사망률 (2026-09-04 신설)
  view_mortality(panel)
  view_public(panel)
  hm <<- build_healthmap()
  view_local(hm)
