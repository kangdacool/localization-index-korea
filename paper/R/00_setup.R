...
write_figure_legends <- function() {
  ...
  out <- c("# Figure legends",
           "",
           paste("*Generated by `write_figure_legends()`. IJEqH 는 그림 제목·범례를 «본문»에",
                 "두라고 규정하므로 그래픽 파일에서 벗겨 여기로 보냈다. ⛔ 손으로 고치지 말 것 —",
                 "정본은 각 스크립트의 `labs()` 다.*"), "")
  for (k in ks) {
    g <- get(k, envir = .FIG_LEGENDS)
    lab <- FIG_LABEL[[as.character(g$n)]]
    out <- c(out, sprintf("**%s %s** %s",
                          if (startsWith(lab, "S")) "Supplementary Fig." else "Fig.",
                          lab, g$title))
    if (!is.null(g$subtitle)) out <- c(out, sprintf("%s", g$subtitle))
    if (!is.null(g$caption))  out <- c(out, sprintf("%s", gsub("\n", " ", g$caption)))
    out <- c(out, "")
  }
